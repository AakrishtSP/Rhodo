...
export namespace Rhodo::Asserts {
    ...
    void debugCheck(bool condition, const char *expr, const std::source_location &loc = std::source_location::current(),
    ...
    void invariant(bool condition, const char *expr, const std::source_location &loc = std::source_location::current(),
                   const char *message = nullptr) noexcept;
    ...
    public:
        ...
        ~ScopedTimer() noexcept
        {
            if constexpr (isDebug) {
                ...
                Logger::coreLogger::debug("[TIMER] {}: {} μs (at {}:{})", name_, duration.count(), loc_.file_name(),
                                          loc_.line());
            }
        }
    ...
    public:
        ...
        static size_t totalAllocated() noexcept
        {
            return totalAllocated_.load(std::memory_order_relaxed);
        }

        static size_t totalFreed() noexcept
        {
            return totalFreed_.load(std::memory_order_relaxed);
        }

        static size_t netAllocated() noexcept
        {
            return totalAllocated() - totalFreed();
        }

        static size_t allocationCount() noexcept
        {
            return allocationCount_.load(std::memory_order_relaxed);
        }

        static size_t deallocationCount() noexcept
        {
        ...
        {
            Logger::coreLogger::info("[MEMORY] Allocated: {} bytes ({} allocs), "
                                     "Freed: {} bytes ({} frees), Net: {} bytes",
                                     totalAllocated(), allocationCount(), totalFreed(), deallocationCount(),
                                     netAllocated());
        }
...
namespace Rhodo::Asserts {
    ...
    {
        Logger::coreLogger::error("[VERIFY FAILED] {} at {}:{}{}{}", expr ? expr : "(null)", loc.file_name(),
                                  loc.line(), message ? " - " : "", message ? message : "");
    }

    static inline void ensureDefaultsInstalled() noexcept
    {
        if (!Detail::gPanicHandler)
            Detail::gPanicHandler = &defaultPanicHandler;
        if (!Detail::gReportHandler)
            Detail::gReportHandler = &defaultReportHandler;
    }
                    ...
                    const char *message) noexcept
    {
        if constexpr (!isDebug) {
            ...
            if (!condition) {
                ensureDefaultsInstalled();
                if (Detail::gPanicHandler)
                    Detail::gPanicHandler(expr, loc, message);
                std::abort();
            }
        }
    }

    void verifyCheck(const bool condition, const char *expr, const std::source_location &loc,
                     const char *message) noexcept
    {
        if (!condition) {
            ensureDefaultsInstalled();
            if (Detail::gReportHandler)
                Detail::gReportHandler(expr, loc, message);
        }
    }
    ...
    void precondition(const bool condition, const char *expr, const std::source_location &loc,
                      const char *message) noexcept
    {
        debugCheck(condition, expr, loc, message);
    }

    void postcondition(const bool condition, const char *expr, const std::source_location &loc,
                       const char *message) noexcept
    {
        debugCheck(condition, expr, loc, message);
    }

    void invariant(const bool condition, const char *expr, const std::source_location &loc,
                   const char *message) noexcept
    {
        debugCheck(condition, expr, loc, message);
    }
    ...
    [[noreturn]] void panic(const char *message, const std::source_location &loc) noexcept
    {
        ensureDefaultsInstalled();
        if (Detail::gPanicHandler)
            Detail::gPanicHandler("panic()", loc, message);
        std::abort();
    }
