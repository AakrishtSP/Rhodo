...
static void defaultPanicHandler(const char* expr,
                                const std::source_location& loc,
                                const char* message) noexcept {
...
static void defaultReportHandler(const char* expr,
                                 const std::source_location& loc,
                                 const char* message) noexcept {
  Logger::coreLogger::error("[VERIFY FAILED] {} at {}:{}{}{}",
                            expr ? expr : "(null)", loc.file_name(), loc.line(),
...
void verifyCheck(const bool condition, const char* expr,
                 const std::source_location& loc,
                 const char* message) noexcept {
  if (!condition) {
    ensureDefaultsInstalled();
...
void precondition(const bool condition, const char* expr,
                  const std::source_location& loc,
                  const char* message) noexcept {
  debugCheck(condition, expr, loc, message);
}

void postcondition(const bool condition, const char* expr,
                   const std::source_location& loc,
                   const char* message) noexcept {
  debugCheck(condition, expr, loc, message);
}

void invariant(const bool condition, const char* expr,
               const std::source_location& loc, const char* message) noexcept {
  debugCheck(condition, expr, loc, message);
}
